              ...
              >
                ...
                <div className="flex flex-wrap gap-2 font-sans">
                  {cat.skills.map((skill, sIdx) => (
                    <span
                      key={sIdx}
                      className="text-[10px] sm:text-xs text-text-secondary px-3 py-1.5 rounded-lg bg-secondary/30 border border-white/5 hover:border-accent/30 hover:text-white transition-all duration-300 relative overflow-hidden group/badge cursor-default"
                    >
                      {skill}
                      {/* Interactive glow dot on badge */}
                      <span className="absolute bottom-0 right-0 w-1 h-1 bg-accent rounded-full opacity-0 group-hover/badge:opacity-100 transition-opacity" />
                    </span>
                  ))}
                </div>
              </motion.div>
